export const STRINGS = {
  ...
  // Chrome for the "What's next?" table printed after a command finishes. The
  // suggestions themselves are generated by the LLM (it understands the SPEX
  // flow and proposes the next command); this section only holds the framing
  // copy. Rendered by `ui/next-steps.ts`.
  nextSteps: {
    thinking: '\nThinking about what to do next...',
    header: "What's next?",
    commandColumn: 'COMMAND',
    descriptionColumn: 'WHY',
    hint: 'Tip: append --help to any command (e.g. `spex implement --help`) to see all options.',
    unavailable:
      "\n(Couldn't generate next-step suggestions right now — run `spex --help` to see every command.)\n",
  },
